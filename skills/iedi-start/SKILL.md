---
name: iedi-start
description: "IEDIセッションを開始する。~/iedi/digest/のカテゴリファイルから選択してintentを確定し、iedi startを実行する。"
---
...
各 `IEDI-*.md` ファイルの最初の行（`# IEDI ドメイン知識: ...`）を読み込んで説明として使う。
